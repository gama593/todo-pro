...
export interface Recurrence { frequency: "daily" | "weekly" | "monthly" | "yearly"; interval: number; weekdays?: number[]; endDate?: string; count?: number; dayOfMonth?: number; }
...
export class TodoDatabase extends Dexie {
  ...
  constructor() {
    super("TodoProScratch");
    this.version(1).stores({ tasks: "id,completed,status,dueDate,projectId,parentTaskId,createdAt,updatedAt,*tags", projects: "id,name,archived", tags: "id,name" });
    this.version(2).stores({ tasks: "id,completed,status,dueDate,projectId,parentTaskId,createdAt,updatedAt,*tags", projects: "id,name,archived,createdAt", tags: "id,name,createdAt" }).upgrade(async tx => {
      const nowValue = new Date().toISOString();
      await tx.table("tasks").toCollection().modify((task: Partial<Task>) => { task.updatedAt ??= task.createdAt ?? nowValue; task.tags ??= []; task.order ??= 0; task.status ??= task.completed ? "done" : "todo"; });
    });
  }
}
